...
`index.js` contains all of our direct database interactions. This is where we `create`, `retrieve`, `update`, and `remove` any data.

Ideally, in the future, `connect.js` and `index.js` should be moved into a separate repository and treated as part of a separate library (to be used in other projects as well).
