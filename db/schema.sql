...
CREATE TABLE Notes{
    ...
    constraint SurahLessonFk foreign key (surah) references Surah(title) on update cascade on delete cascade
}
...
CREATE TABLE LessonSurah {
    ...
    constraint SurahLessonFk foreign key (surah_number) references Surah(title) on update cascade on delete cascade
}
